...
| `ANTHROPIC_CALENDAR_MODEL` | no | = `ANTHROPIC_MODEL` | Model that writes the advice/quip block under a reminder (the event list itself is deterministic). Text the user reads, so it defaults to the main model; set only to override |
