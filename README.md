...
| `ANTHROPIC_PRECISE_MODEL` | no | `claude-opus-5` | The «precise» tier for user-facing text where a wrong detail costs more than the tokens (calendar digest advice). Tiny volume, strongest model |
| `ANTHROPIC_CALENDAR_MODEL` | no | = `ANTHROPIC_PRECISE_MODEL` | Model that writes the advice/quip block under a reminder (the event list itself is deterministic). Set only to override the precise tier for this one use |
...
| `AERODATABOX_API_KEY` | no | — | **Preferred provider**: AeroDataBox — the only feed with real *Boarding/GateClosed* statuses (where the airport publishes them); free tier 600 units/mo, Pro ~$5/mo. Wins over the other two when set; if it fails (lapsed subscription, timeout) the request falls through to the next configured feed |
