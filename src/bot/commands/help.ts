...
export async function cmdHelp(ctx: Context): Promise<void> {
  ...
  await ctx.reply(
    [
      ...
      '• Дружить с Google Календарём: подключи его секретной iCal-ссылкой (/calendar) — буду сам напоминать о событиях (вечером про завтра, утром про сегодня, и незадолго до) и отвечать на «что у меня завтра?».',
      '',
      'Команды:',
      ...
      '/calendar — календари чата; /calendar add <секретная-ics-ссылка> [имя] — подключить; /calendar del <id> — отключить; /calendar check — обновить сейчас',
