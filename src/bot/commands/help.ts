...
export async function cmdHelp(ctx: Context): Promise<void> {
  const adminSection =
    ctx.from && isAdmin(ctx.from.id)
      ? [
          ...
          '/mode <chatId> tutor|secretary|dota — режим чата (репетитор / обычный / дота-пингер)',
  ...
  await ctx.reply(
    [
      ...
      '/dota — пингануть состав; /dota <список> — другой список; редактировать: /dota add|del [список] @ник …, все списки — /dota lists',
